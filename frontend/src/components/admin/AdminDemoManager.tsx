...
const API_DEMOS = 'http://localhost:5000/api/v1/demos';
...
export const AdminDemoManager: React.FC = () => {
  ...
  const openEditModal = (demo: ProductDemo) => {
    ...
    setCategory(demo.category || 'General');
    setOrder(demo.order);
  ...
  const toggleActive = async (demo: ProductDemo) => {
    try {
      const res = await fetch(`${API_DEMOS}/${demo.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const executeDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_DEMOS}/${deleteConfirm.id}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-100/80 dark:bg-slate-950/80 backdrop-blur-md">
            ...
            >
              ...
              <form onSubmit={handleSaveSubmit} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-extrabold text-slate-600 dark:text-slate-300">Description</label>
                  <textarea
                    rows={2}
