...
const API_MEDIA = 'http://localhost:5000/api/v1/uploads/media';
const API_FOLDERS = 'http://localhost:5000/api/v1/uploads/folders';
...
export const AdminMediaLibrary: React.FC = () => {
  ...
  const handleReplaceSubmit = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch(`${API_MEDIA}/${replaceItem.id}`, {
        method: 'PUT',
        body: formData,
      });
      const data = await res.json();
  ...
  const executeDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_MEDIA}/${deleteConfirm.id}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
