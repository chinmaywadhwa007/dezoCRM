...
export const AdminProductManager: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (modal?.mode === 'create') {
        ...
        res = await fetch(`${API}/${modal?.product?.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      }
      const data = await res.json();
      if (data.success) {
        await fetchProducts();
  ...
  const handleToggleEnabled = async (id: string, title: string, current: boolean) => {
    ...
    try {
      await fetch(`${API}/${id}/toggle-enabled`, { method: 'PATCH' });
