...
const API_ABOUT = 'http://localhost:5000/api/v1/about';
const API_MEDIA = 'http://localhost:5000/api/v1/uploads';
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (json.success && uploadedUrl) {
        ...
        const finalUrl = uploadedUrl.startsWith('/') ? `http://localhost:5000${uploadedUrl}` : uploadedUrl;
        const updated = {
          ...formData,
          mediaUrl: finalUrl,
                    ...
                    {formData.mediaUrl && (
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            await fetch('http://localhost:5000/api/v1/about/media', { method: 'DELETE' });
                          } catch {
                            // ignore
                          }
                ...
                {mediaList.map((item) => (
                  ...
                  >
                    {item.mimeType?.startsWith('video/') || item.url?.endsWith('.mp4') ? (
                      <video src={item.url.startsWith('/') ? `http://localhost:5000${item.url}` : item.url} className="w-full h-full object-cover" />
                    ) : (
                      <img src={item.url.startsWith('/') ? `http://localhost:5000${item.url}` : item.url} alt={item.filename} className="w-full h-full object-cover" />
                    )}
